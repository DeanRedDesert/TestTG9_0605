//-----------------------------------------------------------------------
// <copyright file = "UgpProgressiveInterfaceConfiguration.cs" company = "IGT">
...
namespace IGT.Game.Core.Communication.Foundation.InterfaceExtensions.Ugp.Progressive
{
    ...
    /// <summary>
    /// Configuration for the UgpProgressive extended interface.
    /// </summary>
    public sealed class UgpProgressiveInterfaceConfiguration : F2XInterfaceExtensionConfigurationBase
    {
        ...
        public UgpProgressiveInterfaceConfiguration(bool required)
        {
        ...
        public override IInterfaceExtension CreateInterfaceExtension(IInterfaceExtensionDependencies dependencies)
        {
            switch(dependencies)
            {
                ...
                case F2XInterfaceExtensionDependencies f2XDependencies:
                    try
                    {
                        return new F2XUgpProgressive(f2XDependencies.Category as IUgpProgressiveCategory);
                    }
                    catch(Exception innerException)
                    {
                        throw new InterfaceExtensionDependencyException(
                            "Issue constructing F2X UgpProgressive implementation from dependencies.", innerException);
                    }
                    
                case StandaloneInterfaceExtensionDependencies _:
                    try
                    {
                        return new StandaloneUgpProgressive();
                    }
                    catch(Exception innerException)
                    {
                        throw new InterfaceExtensionDependencyException(
                            "Issue constructing standalone UgpProgressive implementation from dependencies.", innerException);
                    }
        ...
        /// <inheritdoc/>
        public override Type InterfaceType => typeof(IUgpProgressive);
        ...
        public override IEnumerable<CategoryRequest> GetCategoryRequests()
        {
            return new List<CategoryRequest>
                       {
                           new CategoryRequest(
                               new CategoryVersionInformation((int)MessageCategory.UgpProgressive,
                                                              RequestedMajorVersion,
                                                              RequestedMinorVersion),
                               required,
                               FoundationTarget.AllAscent,  //TODO: Revised to target Foundation in need
                               dependencies => new UgpProgressiveCategory(dependencies.Transport))
                       };
        }
