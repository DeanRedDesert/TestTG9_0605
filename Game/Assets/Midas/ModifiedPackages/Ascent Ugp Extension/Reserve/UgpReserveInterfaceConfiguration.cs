//-----------------------------------------------------------------------
// <copyright file = "UgpReserveInterfaceConfiguration.cs" company = "IGT">
...
namespace IGT.Game.Core.Communication.Foundation.InterfaceExtensions.Ugp.Reserve
{
    ...
    /// <summary>
    /// Configuration for the UgpReserve extended interface.
    /// </summary>
    public sealed class UgpReserveInterfaceConfiguration : F2XInterfaceExtensionConfigurationBase
    {
        ...
        public UgpReserveInterfaceConfiguration(bool required)
        {
        ...
        public override IInterfaceExtension CreateInterfaceExtension(IInterfaceExtensionDependencies dependencies)
        {
            switch(dependencies)
            {
                ...
                case F2XInterfaceExtensionDependencies f2XDependencies:
                    try
                    {
                        return new F2XUgpReserve(f2XDependencies.Category as IUgpReserveCategory,
                            f2XDependencies.TransactionalEventDispatcher);
                    }
                    catch(Exception innerException)
                    {
                        throw new InterfaceExtensionDependencyException(
                            "Issue constructing F2X UgpReserve implementation from dependencies.", innerException);
                    }

                case StandaloneInterfaceExtensionDependencies standaloneDependencies:
                    try
                    {
                        return new StandaloneUgpReserve(standaloneDependencies.EventPoster,
                            standaloneDependencies.TransactionalEventDispatcher);
                    }
                    catch(Exception innerException)
                    {
                        throw new InterfaceExtensionDependencyException(
                            "Issue constructing standalone UgpReserve implementation from dependencies.", innerException);
                    }
        ...
        /// <inheritdoc/>
        public override Type InterfaceType => typeof(IUgpReserve);
        ...
        public override IEnumerable<CategoryRequest> GetCategoryRequests()
        {
            return new List<CategoryRequest>
                       {
                           new CategoryRequest(
                               new CategoryVersionInformation((int)MessageCategory.UgpReserve,
                                                              RequestedMajorVersion,
                                                              RequestedMinorVersion),
                               required,
                               FoundationTarget.AllAscent,  //TODO: Revised to target Foundation in need
                               dependencies => new UgpReserveCategory(
                                   dependencies.Transport, new UgpReserveCategoryCallbacks(dependencies.EventCallbacks)))
                       };
        }
