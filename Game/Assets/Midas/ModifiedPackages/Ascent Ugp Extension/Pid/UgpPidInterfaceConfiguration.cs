//-----------------------------------------------------------------------
// <copyright file = "UgpPidInterfaceConfiguration.cs" company = "IGT">
...
namespace IGT.Game.Core.Communication.Foundation.InterfaceExtensions.Ugp.Pid
{
    ...
    /// <summary>
    /// Configuration for the UgpPid extended interface.
    /// </summary>
    public sealed class UgpPidInterfaceConfiguration : F2XInterfaceExtensionConfigurationBase
    {
        ...
        /// <summary>
        /// Constructs an instance of this interface configuration.
        /// </summary>
        /// <param name="required">The flag indicating if this extended interface is mandatory.</param>
        public UgpPidInterfaceConfiguration(bool required)
        {
        ...
        public override IInterfaceExtension CreateInterfaceExtension(IInterfaceExtensionDependencies dependencies)
        {
            switch(dependencies)
            {
                ...
                case F2XInterfaceExtensionDependencies f2XDependencies:
                    try
                    {
                        return new F2XUgpPid(f2XDependencies.Category as IUgpPidCategory,
                            f2XDependencies.TransactionalEventDispatcher);
                    }
                    catch(Exception innerException)
                    {
                        throw new InterfaceExtensionDependencyException(
                            "Issue constructing F2X UgpPid implementation from dependencies.", innerException);
                    }

                case StandaloneInterfaceExtensionDependencies standaloneDependencies:
                    try
                    {
                        return new StandaloneUgpPid(standaloneDependencies.EventPoster,
                            standaloneDependencies.TransactionalEventDispatcher);
                    }
                    catch(Exception innerException)
                    {
                        throw new InterfaceExtensionDependencyException(
                            "Issue constructing standalone UgpPid implementation from dependencies.", innerException);
                    }
        ...
        /// <inheritdoc/>
        public override Type InterfaceType => typeof(IUgpPid);
        ...
        public override IEnumerable<CategoryRequest> GetCategoryRequests()
        {
            return new List<CategoryRequest>
                       {
                           new CategoryRequest(
                               new CategoryVersionInformation((int)MessageCategory.UgpPid,
                                                              RequestedMajorVersion,
                                                              RequestedMinorVersion),
                               required,
                               FoundationTarget.AllAscent,  //TODO: Revised to target Foundation in need
                               dependencies => new UgpPidCategory(
                                   dependencies.Transport, new UgpPidCategoryCallbacks(dependencies.EventCallbacks)))
                       };
        }
