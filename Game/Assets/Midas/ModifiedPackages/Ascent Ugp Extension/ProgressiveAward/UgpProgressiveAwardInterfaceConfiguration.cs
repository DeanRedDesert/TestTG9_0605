//-----------------------------------------------------------------------
// <copyright file = "UgpProgressiveAwardInterfaceConfiguration.cs" company = "IGT">
...
namespace IGT.Game.Core.Communication.Foundation.InterfaceExtensions.Ugp.ProgressiveAward
{
    ...
    /// <summary>
    /// Configuration for the UgpProgressiveAward extended interface.
    /// </summary>
    public sealed class UgpProgressiveAwardInterfaceConfiguration : F2XInterfaceExtensionConfigurationBase
    {
        ...
        public UgpProgressiveAwardInterfaceConfiguration(bool required)
        {
        ...
        public override IInterfaceExtension CreateInterfaceExtension(IInterfaceExtensionDependencies dependencies)
        {
            switch(dependencies)
            {
                ...
                case F2XInterfaceExtensionDependencies f2XDependencies:
                    try
                    {
                        return new F2XUgpProgressiveAward(f2XDependencies.Category as IUgpProgressiveAwardCategory,
                            f2XDependencies.TransactionalEventDispatcher);
                    }
                    catch(Exception innerException)
                    {
                        throw new InterfaceExtensionDependencyException(
                            "Issue constructing F2X UgpProgressiveAward implementation from dependencies.", innerException);
                    }

                case StandaloneInterfaceExtensionDependencies standaloneDependencies:
                    try
                    {
                        return new StandaloneUgpProgressiveAward(standaloneDependencies.EventPoster,
                            standaloneDependencies.TransactionalEventDispatcher);
                    }
                    catch(Exception innerException)
                    {
                        throw new InterfaceExtensionDependencyException(
                            "Issue constructing standalone UgpProgressiveAward implementation from dependencies.", innerException);
                    }
        ...
        /// <inheritdoc/>
        public override Type InterfaceType => typeof(IUgpProgressiveAward);
        ...
        public override IEnumerable<CategoryRequest> GetCategoryRequests()
        {
            return new List<CategoryRequest>
                       {
                           new CategoryRequest(
                               new CategoryVersionInformation((int)MessageCategory.UgpProgressiveAward,
                                                              RequestedMajorVersion,
                                                              RequestedMinorVersion),
                               required,
                               FoundationTarget.AllAscent,  //TODO: Revised to target Foundation in need
                               dependencies => new UgpProgressiveAwardCategory(
                                   dependencies.Transport, new UgpProgressiveAwardCategoryCallbacks(dependencies.EventCallbacks)))
                       };
        }
