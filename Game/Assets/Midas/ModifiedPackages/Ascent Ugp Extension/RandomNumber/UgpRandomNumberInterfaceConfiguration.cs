//-----------------------------------------------------------------------
// <copyright file = "UgpRandomNumberInterfaceConfiguration.cs" company = "IGT">
...
namespace IGT.Game.Core.Communication.Foundation.InterfaceExtensions.Ugp.RandomNumber
{
    ...
    /// <summary>
    /// Configuration for the UgpRandomNumber extended interface.
    /// </summary>
    public sealed class UgpRandomNumberInterfaceConfiguration : F2XInterfaceExtensionConfigurationBase
    {
        ...
        public UgpRandomNumberInterfaceConfiguration(bool required)
        {
        ...
        public override IInterfaceExtension CreateInterfaceExtension(IInterfaceExtensionDependencies dependencies)
        {
            switch(dependencies)
            {
                ...
                case F2XInterfaceExtensionDependencies f2XDependencies:
                    try
                    {
                        return new F2XUgpRandomNumber(f2XDependencies.Category as IUgpRandomNumberCategory);
                    }
                    catch(Exception innerException)
                    {
                        throw new InterfaceExtensionDependencyException(
                            "Issue constructing F2X UgpRandomNumber implementation from dependencies.", innerException);
                    }
                    
                case StandaloneInterfaceExtensionDependencies _:
                    try
                    {
                        return new StandaloneUgpRandomNumber();
                    }
                    catch(Exception innerException)
                    {
                        throw new InterfaceExtensionDependencyException(
                            "Issue constructing standalone UgpRandomNumber implementation from dependencies.", innerException);
                    }
        ...
        /// <inheritdoc/>
        public override Type InterfaceType => typeof(IUgpRandomNumber);
        ...
        public override IEnumerable<CategoryRequest> GetCategoryRequests()
        {
            return new List<CategoryRequest>
                       {
                           new CategoryRequest(
                               new CategoryVersionInformation((int)MessageCategory.UgpRandomNumber,
                                                              RequestedMajorVersion,
                                                              RequestedMinorVersion),
                               required,
                               FoundationTarget.AllAscent,  //TODO: Revised to target Foundation in need
                               dependencies => new UgpRandomNumberCategory(dependencies.Transport))
                       };
        }
